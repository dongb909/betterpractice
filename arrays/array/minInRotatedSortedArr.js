...
var findMin = function(nums) {
  ...
  while (l < r) { //l will always be less than r index untill r=l, r will never > l
    ...
    if (nums[m] > nums[r]) l = m+1 //because we already established that m is not the smallest number so can rule it out
    else  r = m //if mid is < right or equal to right and it's sorted, then all nums to the right of mid is just going up so want to check to left of mid
   //num[m] is smallest so far so keep it in next search
